...
{
    private const string _usersInGroupUri = "api/v1/users/in_group/";
    private const string _messageUri = "api/v1/messages.json";
    ...
    private async Task<List<YammerUser>> GetAllMembers(CancellationToken cancellationToken)
    {
        ...
        return members.Where(x => !x.Name.Contains("backup")).ToList();
    }
    ...
    private async Task<YammerUsers> GetPage(int page, CancellationToken cancellationToken) =>
        await DeserializeResponse(
            await _client.GetAsync(
                new Uri($"{_usersInGroupUri}{_id}?page={page}", UriKind.Relative),
                cancellationToken),
            cancellationToken);
    ...
    private async Task PostMatches(string formattedMatches, int pageNo, int pages, CancellationToken cancellationToken)
    {
        ...
        var response = await _client.PostAsync(new Uri(_messageUri, UriKind.Relative), content, cancellationToken);
